...
def TarentatekAttack(player):
    ...
    while thealth > 0:
        ...
        if atype == '2':
            ...
            print "The tarentatek takes {} damage and now has {} health.".format(player.damage, thealth)
        if thealth < 0:
                thealth = 0
                print "It's dead."
                HealChoice(player)
            ...
            if player.health < 0:
                ...
                print "GAME OVER"
                exit()
            print "The tarentatek attacks you for {} damage, leaving you with {} health.".format(tattack, player.health)
        HealChoice(player)

def TukataAttack(player):
    thealth = 150
    print "The tuk'ata attacks!"
    while player.health > 0 or thealth > 0:
        print "You attack first!"
        print "Try for a headshot or just attack?"
        print "You might miss of you go for a headshot, but there's higher damage."
        atype = raw_input("1 for headshot, 2 for attack: ")
        ...
        if atype == '2':
            thealth -= player.damage
            print "The tuk'ata takes {} damage and now has {} health.".format(player.damage, thealth)
        if thealth < 0:
                thealth = 0
                print "It's dead."
                HealChoice(player)
            ...
            if player.health < 0:
                ...
                print "GAME OVER"
                exit()
            print "The tuk'ata attacks you for {} damage, leaving you with {} health.".format(tattack, player.health)
        HealChoice(player)
...
This place is filled with darkness. Do you embrace it?"""
    ...
    if choice == 1:
        LDNpoints(player, 40, 0, 15)
    if choice == 2:
        LDNpoints(player, 0, 0, 55)
    if choice == 3:
        LDNpoints(player, 0, 40, 15)
...
You wonder if it could do the same for you. The tomb's power is... alluring."""
        ...
        if yaynay == 2:
            LDNpoints(player, 0, 30, 10)
        else:
            LDNpoints(player, 30, 0, 10)
        print "..."
    ...
    if pick == 3:
        ...
        while crypt == '2':
            ...
            if loot > 80:
                ...
                LDNpoints(player, 30, 30, 0)
            else:
                print "You found some Grey Jedi Holocrons!"
                print "These are rare."
                LDNpoints(player, 0, 0, math.floor(random.random() * 300))
